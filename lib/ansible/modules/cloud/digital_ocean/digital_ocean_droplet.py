...
options:
  ...
  size:
    description:
     - The slug of the desired droplet configuration, e.g. 's-1vcpu-1gb', '2gb', 'c-32vcpu-64gb', or 's-32vcpu-192gb'.
    aliases: ['size_id']
    default: 's-1vcpu-1gb'
  image:
    description:
  ...
  region:
    description:
     - The slug of the datacenter you would like your droplet to be created in, e.g. 'sfo2', 'ams3', or 'sgp1'.
    aliases: ['region_id']
  ssh_keys:
    description:
...
class DODroplet(object):
    def __init__(self, module):
        ...
        if self._id and self._name:
            self.module.warn("Both id {} and name {} supplied. Your play may turn unexpectedly!".
    ...
    def get(self):
        ...
        if json_data and not self.rebuild:
            ...
            self.module.fail_json(changed=False, msg='droplet {} not found. Rebuild failed.'.format(self._droplet))
        ...
        if self._id:
            self.module.warn("Trying to build {}. Parameter id is found, it makes no sense here!".format(self._name))
        if not _size:
            _default_size = 's-1vcpu-1gb'
            self.module.warn("Missing 'size' parameter. Using size={}".format(_default_size))
            _size = _default_size
        if not self._params_ok(self._name, _image, _region):
            self.module.fail_json(changed=False, msg='Droplet not created. Not enough parameters: name={} region={}'
                                                     ' image={} size={}'.format(self._name, _region, _image, _size))
        ...
        self.module.exit_json(changed=True, data=self.expose_addresses(response.json))

    def _rebuild(self, json_data):
        if self._id and self._name:
            self.module.warn("Trying to rebuild droplet id={}. Parameter name is found too, it makes no sense here!".format(self._id))
        if self.module.check_mode:
            self.module.exit_json(changed=True)
        ...
        droplet_data = self.expose_addresses(json_data)
